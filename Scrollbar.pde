class HScrollbar {
  ...
  */
  HScrollbar (float x, float y, float w, float h) {
    ...
    sliderPositionMin = xPosition - barWidth/2 + barHeight/2;
    sliderPositionMax = xPosition + barWidth/2 - barHeight/2;
  }
  /**
  ...
  */
  boolean isMouseOver() {
    if (mouseX > sliderPositionMin - barHeight/2 && mouseX < sliderPositionMax + barHeight/2 &&
      mouseY > yPosition - barHeight/2  && mouseY < yPosition+barHeight/2) {
      return true;
    }
    else {
      return false;
  ...
  */
  float getPos() {
    return (sliderPosition - xPosition)/(barWidth - barHeight) + 0.5;
  }
}